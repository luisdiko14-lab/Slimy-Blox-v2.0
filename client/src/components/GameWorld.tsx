...
export function GameWorld() {
  ...
  const tutorialSteps = [
    {
      ...
      content: "Welcome to ADMIN WORLD. Here, power is absolute. You have been granted OWNER status.",
    },
    {
      title: "MOVEMENT PROTOCOL",
      content: "Use WASD keys to navigate the grid. On mobile, use the D-pad below.",
    },
    {
      title: "COMMAND INPUT",
      content: "Press '/' to open the command console. This is your interface for control.",
    },
    {
      title: "ADMIN POWERS",
      content: "Try commands like /speed 10, /size 50, or /fly. Type /help for all commands.",
    },
    {
      title: "GLOBAL BROADCAST",
      content: "Use /announce [message] to send a system-wide alert to all players.",
    },
  ];

  const handleNextTutorial = () => {
    console.log(`[Tutorial] Advancing to step ${tutorialStep + 1}`);
  ...
  const handleCustomRankSubmit = () => {
    ...
    if (urlParams.has('ip')) {
      // Trigger tutorial for new players if skipping server browser
  ...
  useEffect(() => {
    ...
    console.log(`User's plan: ${currentPlan}`);
  ...
  useEffect(() => {
    ...
    ws.onmessage = (event) => {
      try {
        ...
        if (msg.type !== "PLAYER_STATE") console.log(`[WebSocket] Client received: ${msg.type}`);
        ...
        if (msg.type === "PLAYER_STATE") {
          ...
          console.log(`[WebSocket] Player received KICK_ALL, reason: ${reason}`);
          if (reason === "banned") {
            window.location.href = "/banned.html";
          } else if (reason === "beluga") {
            window.location.href = "/bossofgodofdabelucat.html";
          } else if (reason === 'kill') {
            console.log("[Death] Initializing death sequence animation");
  ...
  const executeCommand = (cmd: string, args: string[]) => {
    ...
    console.log(`${player.name} ran ${cmd} ${commandInput}`);
    playClickSound();
    ...
    switch (cmd) {
      ...
      case "kick":
        ...
        if (kickTarget !== "@everyone") {
          const targetExists = Array.from(otherPlayers.values()).some(p => p.name.toLowerCase() === kickTarget.toLowerCase());
          if (!targetExists) {
            console.warn(`User ran command kick ${kickTarget} but the user couldn't get kicked because they're not in the server`);
            return addToChat(`Target ${kickTarget} not found.`, "error");
          }
        }
      ...
      case "kill":
        ...
        if (killTarget !== "@everyone") {
          const targetExists = Array.from(otherPlayers.values()).some(p => p.name.toLowerCase() === killTarget.toLowerCase());
          if (!targetExists) {
            console.warn(`User ran command kill ${killTarget} but the user couldn't get killed because they're not in the server`);
            return addToChat(`Target ${killTarget} not found.`, "error");
          }
        }
      ...
      case "rank":
        ...
        if (rankTarget !== "@everyone") {
          const targetExists = Array.from(otherPlayers.values()).some(p => p.name.toLowerCase() === rankTarget.toLowerCase());
          if (!targetExists) {
            console.warn(`User ran command rank ${rankName} ${rankTarget} but the user couldn't get ranked because they're not in the server`);
            return addToChat(`Target ${rankTarget} not found.`, "error");
          }
        }
    ...
    <div className="relative w-full h-screen bg-black overflow-hidden font-terminal">
      ...
      <AnimatePresence>
        {showTutorial && (
          ...
          >
            <div className="max-w-md w-full border-2 border-[#0f0] bg-black p-6 shadow-[0_0_20px_rgba(0,255,0,0.3)]">
              <div className="flex items-center gap-2 mb-4 text-[#0f0]">
              ...
              <p className="text-[#0f0]/80 mb-6 font-mono leading-relaxed">
                {tutorialSteps[tutorialStep].content}
              </p>

              <div className="flex justify-between items-center">
                <span className="text-[#0f0]/40 text-xs font-mono">
                  STEP {tutorialStep + 1} OF {tutorialSteps.length}
                </span>
                <Button 
                  onClick={handleNextTutorial}
                  className="bg-[#0f0] text-black hover:bg-[#0f0]/80 font-bold px-8"
                >
                  {tutorialStep === tutorialSteps.length - 1 ? "ENTER WORLD" : "NEXT >"}
                </Button>
              </div>
            </div>
          </motion.div>
            ...
            <div className="w-full max-w-md retro-container border-2 border-primary p-6 bg-black relative">
              ...
              <div className="flex flex-col gap-4">
                ...
                <Button 
                  ...
                  variant="outline"
                  className="w-full border-primary text-primary font-pixel hover:bg-primary/20 transition-all mt-2"
                >
                  [ ACCESS_PREMIUM_SHOP ]
                </Button>
              </div>
      ...
      <AnimatePresence>
        {showDeathScreen && (
          <motion.div
            ...
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
            className="fixed inset-0 z-[300] bg-red-950/90 flex items-center justify-center backdrop-blur-sm"
          >
            <div className="text-center p-8 border-4 border-red-500 bg-black shadow-[0_0_50px_rgba(255,0,0,0.5)]">
              <motion.h1 
                initial={{ scale: 0.5, opacity: 0 }}
                animate={{ scale: 1, opacity: 1 }}
                className="text-6xl font-black text-red-500 mb-8 font-pixel tracking-tighter italic"
              >
                YOU WERE KILLED
              </motion.h1>
              <p className="text-red-400 mb-8 font-mono">SYSTEM_HALTED: REBOOT_REQUIRED</p>
              <Button 
                onClick={() => {
                  setShowDeathScreen(false);
                  setPlayer(p => ({ ...p, pos: { x: MAP_WIDTH / 2, y: MAP_HEIGHT / 2 } }));
                  addToChat("SYSTEM: Respawn successful.", "info");
                }}
                className="bg-red-600 text-white hover:bg-red-500 px-12 py-6 text-xl font-bold"
              >
                RESPAWN_INITIALIZE
              </Button>
            </div>
